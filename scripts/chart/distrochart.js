/**
 * @fileOverview A d3v2 based distribution chart system. Supports: Box plots, Violin plots, Notched box plots, trend lines, beeswarm plot
 * @version 3.0
 */
...
function makeDistroChart(settings) {
    ...
    chart.settings = {
        ...
        margin: {top: 15, right: 30, bottom: 40, left: 50},
        constrainExtremes: false,
        color: d3v2.scale.category10()
    };
    ...
    function formatAsFloat(d) {
        if (d % 1 !== 0) {
            return d3v2.format(".2f")(d);
        } else {
            return d3v2.format(".0f")(d);
        }
    }
    ...
    function getColorFunct(colorOptions) {
       // colorOptions = ['blue', 'blue', 'red','red', 'red', 'blue', 'blue', 'red', 'blue']
    ...
    function tooltipHover(groupName, metrics) {
        var tooltipString = "Module: " + moduleRef[groupName];
    ...
    !function prepareData() {
        function calcMetrics(values) {
            ...
            /*
            metrics.min = d3v2.min(values) ;
            metrics.quartile1 = d3v2.quantile(values, 0.25);
            metrics.median = d3v2.median(values);
            metrics.mean = d3v2.mean(values);
            metrics.quartile3 = d3v2.quantile(values, 0.75);
            metrics.max = d3v2.max(values);
            metrics.iqr = metrics.quartile3 - metrics.quartile1;
            */

            metrics.min = (isNaN(d3v2.min(values))) ? 0 : d3v2.min(values) ;
            metrics.quartile1 = (isNaN(d3v2.quantile(values, 0.25))) ? 0 : d3v2.quantile(values, 0.25);
            metrics.median = (isNaN(d3v2.median(values))) ? 0 : d3v2.median(values);
            metrics.mean = (isNaN(d3v2.mean(values))) ? 0 : d3v2.mean(values);
            metrics.quartile3 = (isNaN(d3v2.quantile(values, 0.75))) ? 0 : d3v2.quantile(values, 0.75);
            metrics.max = (isNaN(d3v2.max(values))) ? 0 : d3v2.max(values);
            metrics.iqr = metrics.quartile3 - metrics.quartile1;
        ...
        for (current_row = 0; current_row < chart.data.length; current_row++) {
            ...
            current_name = chart.data[current_row]['name'];
            ...
            if (chart.groupObjs.hasOwnProperty(current_x)) {
                ...
                chart.groupObjs[current_x].names = {current_name};
            }
        }

        for (var c = 0; c < 22; c ++) {
            if (!chart.groupObjs.hasOwnProperty(c)) {
                    chart.groupObjs[c] = {};
                    chart.groupObjs[c].values = [];
                    chart.groupObjs[c].names = {current_name};
            }
        ...
        for (var cName in chart.groupObjs) {
            ...
            chart.groupObjs[cName].values.sort(d3v2.ascending);
            chart.groupObjs[cName].metrics = {};
            chart.groupObjs[cName].metrics = calcMetrics(chart.groupObjs[cName].values);

            chart.groupObjs[cName].metrics.readingTime = similarityMap.nodes[cName].reading_time;
    ...
    !function prepareSettings() {
        ...
        if (chart.settings.scale === 'log') {
            chart.yScale = d3v2.scale.log();
            chart.yFormatter = logFormatNumber;
        } else {
            chart.yScale = d3v2.scale.linear();
        }
        ...
        if (chart.settings.constrainExtremes === true) {
            ...
            chart.range = d3v2.extent(fences);

        } else {
            chart.range = d3v2.extent(chart.data, function (d) {return d[chart.settings.yName];});
        }
        ...
        chart.xScale = d3v2.scale.ordinal().domain(Object.keys(chart.groupObjs)).rangeBands([0, chart.width]);
      // console.log(chart.groupObjs)//chart.groupObjs[0].names.current_name
        /*var xdata = [];
        Object.keys(chart.groupObjs).forEach(function(key) {
            xdata.push(chart.groupObjs[key].names.current_name);
        });*/

       var xdata = moduleRef;
        chart.xtcScale = d3v2.scale.ordinal().domain(xdata).rangeBands([0, chart.width]);


        //Build Axes Functions
        chart.objs.yAxis = d3v2.svg.axis()
            .scale(chart.yScale)
            .orient("left")
        ...
        chart.objs.xAxis = d3v2.svg.axis().scale(chart.xtcScale).orient("bottom").tickSize(5);
    }();
    ...
    chart.update = function () {
        ...
        chart.xtcScale.rangeBands([0, chart.width]);
    ...
    !function prepareChart() {
        ...
        chart.objs.mainDiv = d3v2.select(chart.settings.selector)
        ...
        chart.objs.chartDiv = d3v2.select(chart.selector);
        d3v2.select(window).on('resize.' + chart.selector, chart.update);
        ...
        for (var cName in chart.groupObjs) {
            ...
            chart.groupObjs[cName].g.on("mouseover", function () {
                chart.objs.tooltip
                    .style("display", null)
                    .style("left", (d3v2.event.pageX - 300) + "px")
                    .style("top", (d3v2.event.pageY - 98) + "px");
    ...
    chart.renderViolinPlot = function (options) {
        ...
        chart.violinPlots.update = function () {
            ...
            for (cName in chart.groupObjs) {
                ...
                if (vOpts.clamp == 0 || vOpts.clamp == -1) { //
                    ...
                    interpolateMax = d3v2.min(cViolinPlot.kdedata.filter(function (d) {
                        return (d.x > chart.groupObjs[cName].metrics.max && d.y == 0)
                    }), function (d) {
                        return d.x;
                    });
                    // interpolateMin = the Maximum value less than the min where y = 0
                    interpolateMin = d3v2.max(cViolinPlot.kdedata.filter(function (d) {
            ...
            for (cName in chart.groupObjs) {
                ...
                var yVScale = d3v2.scale.linear()
                    .range([width, 0])
                    .domain([0, d3v2.max(cViolinPlot.kdedata, function (d) {return d.y;})])
                    .clamp(true);
        ...
        function kernelDensityEstimator(kernel, x) {
            return function (sample) {
                return x.map(function (x) {
                    return {x:x, y:d3v2.mean(sample, function (v) {return kernel(x - v);})};
                });
            };
        }
        ...
        function eKernelTest(kernel, array) {
            return function (testX) {
                return d3v2.mean(array, function (v) {return kernel(testX - v);})
            }
        }

        chart.violinPlots.prepareViolin();

        d3v2.select(window).on('resize.' + chart.selector + '.violinPlot', chart.violinPlots.update);
        chart.violinPlots.update();
        return chart;
    };
    ...
    chart.renderBoxPlot = function (options) {
        ...
        chart.boxPlots.update = function () {
            ...
            for (cName in chart.groupObjs) {
               ...
               /*
                 ...
                 if(chart.groupObjs[cName].values.length == 0){
                    return
                }
        ...
        d3v2.select(window).on('resize.' + chart.selector + '.boxPlot', chart.boxPlots.update);
        chart.boxPlots.update();
        return chart;
    ...
    chart.renderNotchBoxes = function (options) {
        ...
        d3v2.select(window).on('resize.' + chart.selector + '.notchBox', chart.notchBoxes.update);
        chart.notchBoxes.update();
        return chart;
    };
    ...
    chart.renderDataPlots = function (options) {
                ...
                if (cPlot.objs.points) {
                    if (dOpts.plotType == 'beeswarm') {
                        ...
                        var yPtScale = chart.yScale.copy()
                            ...
                            .interpolate(d3v2.interpolateRound)
        ...
        chart.dataPlots.preparePlots = function () {
            ...
            if (dOpts.showLines && dOpts.showLines.length > 0) {
                ...
                for (var line in dOpts.showLines) {
                    ...
                    chart.dataPlots.objs.lines[cMetric].line = d3v2.svg.line()
                    ...
                    if ( cMetric == "readingTime"){
                        chart.dataPlots.objs.lines[cMetric].g.attr("stroke-width", "4")
                    }

                    // legend

                    var legend_keys = ["Exp. Reading Time", "Median"];
                    var legend_color = ["#2ca02c", "#d52729"];
                    var stroke_thickness = [4, 1];

                    var lineLegend = chart.dataPlots.objs.g.selectAll(".lineLegend").data(legend_keys)
                                    .enter().append("g")
                                    .attr("class","lineLegend")
                                    .attr("font-size", "8px")
                                    .attr("transform", function (d,i) {
                                        return "translate(" + (chart.width - 100)  + "," + (i*15 )+")";
                                    });
                        ...
                        lineLegend.append("line")
                            .style("stroke-width",  function (d,i) {return stroke_thickness[i];})
                            .style("stroke-dasharray", "3")  // <== This line here!!
                            .style("stroke",  function (d,i) {return legend_color[i];})
                            .attr("x1", -10)
                            .attr("x2", 10)
                            .attr("y1", 5)
                            .attr("y2", 5);
        ...
        d3v2.select(window).on('resize.' + chart.selector + '.dataPlot', chart.dataPlots.update);
        chart.dataPlots.update();
        return chart;
    };
     ...
     *
     */
    chart.renderAreaPlot = function (options) {
        chart.AreaPlot = {};
        ...
        chart.AreaPlot.options = shallowCopy(defaultOptions);
        for (var option in options) {
            chart.AreaPlot.options[option] = options[option]
        }
        var dOpts = chart.AreaPlot.options;

        //Create notch objects
        for (var cName in chart.groupObjs) {
            chart.groupObjs[cName].AreaPlot = {};
            chart.groupObjs[cName].AreaPlot.objs = {};
        }
        // The lines don't fit into a group bucket so they live under the dataPlot object
        chart.AreaPlot.objs = {};
         ...
         * @param updateOptions
         */
        chart.AreaPlot.change = function (updateOptions) {
            if (updateOptions) {
                for (var key in updateOptions) {
                    dOpts[key] = updateOptions[key]
                }
            ...
            chart.AreaPlot.objs.g.remove();
            for (var cName in chart.groupObjs) {
                chart.groupObjs[cName].AreaPlot.objs.g.remove()
            }
            chart.AreaPlot.preparePlots();
            chart.AreaPlot.update()
        };

        chart.AreaPlot.reset = function () {
            chart.AreaPlot.change(defaultOptions)
        };
        chart.AreaPlot.show = function (opts) {
            if (opts !== undefined) {
                opts.show = true;
                if (opts.reset) {
                    chart.AreaPlot.reset()
                }
            } else {
                opts = {show: true};
            }
            chart.AreaPlot.change(opts)
        };
        chart.AreaPlot.hide = function (opts) {
            if (opts !== undefined) {
                opts.show = false;
                if (opts.reset) {
                    chart.AreaPlot.reset()
         ...
         * Update the data plot obj values
         */
        chart.AreaPlot.update = function () {
            var cName, cGroup, cPlot;

            // Metrics lines
            if (chart.AreaPlot.objs.g) {
                var halfBand = chart.xScale.rangeBand() / 2; // find the middle of each band
                for (var cMetric in chart.AreaPlot.objs.lines) {
                    chart.AreaPlot.objs.lines[cMetric].line
                        .x(function (d) {
                            return chart.xScale(d.x) + halfBand
                        });

                    chart.AreaPlot.objs.lines[cMetric].g
                        .datum(chart.AreaPlot.objs.lines[cMetric].values)
                        .attr('d', chart.AreaPlot.objs.lines[cMetric].line);
                }
            }

            for (cName in chart.groupObjs) {
                cGroup = chart.groupObjs[cName];
                cPlot = cGroup.AreaPlot;
         ...
         * Create the svg elements for the data plots
         */
        chart.AreaPlot.preparePlots = function () {
            var cName, cPlot;

            if (dOpts && dOpts.colors) {
                chart.AreaPlot.colorFunct = getColorFunct(dOpts.colors);
            } else {
                chart.AreaPlot.colorFunct = chart.colorFunct
            }
            ...
            // Metrics lines
            chart.AreaPlot.objs.g = chart.objs.g.append("g").attr("class", "metrics-area");
            if (dOpts.showLines && dOpts.showLines.length > 0) {
                chart.AreaPlot.objs.lines = {};
                var cMetric;

//Start
                var line = 0;
                cMetric = dOpts.showLines[line];
                chart.AreaPlot.objs.lines[cMetric] = {};
                chart.AreaPlot.objs.lines[cMetric].values = [];
                // get values
                for (var cGroup in chart.groupObjs) {
                        chart.AreaPlot.objs.lines[cMetric].values.push({
                            x: cGroup,
                            y: chart.groupObjs[cGroup].metrics[cMetric]
                        })
                }

                chart.AreaPlot.objs.lines[cMetric].line = d3v2.svg.line()
                        .interpolate("cardinal")
                        .y(function (d) {
                            return chart.yScale(d.y)
                        });
                
                chart.AreaPlot.objs.lines[cMetric].g = chart.AreaPlot.objs.g.append("path")
                            .attr("class", "line " + cMetric)
                            .attr("data-metric", cMetric)
                            .style("fill", 'none')
                            .style("stroke", chart.colorFunct(cMetric));
                ...
                var data1 = chart.AreaPlot.objs.lines[dOpts.showLines[0]].values;
                var a1 = data1.map(({ x, y }) => ({ x: x, z: y }));
                var a2 = chart.AreaPlot.objs.lines[dOpts.showLines[1]].values;

                var mergedList = _.map(a1, function(item){
                     return _.extend(item, _.findWhere(a2, { x: item.x}));
                });

        
                var halfBand = chart.xScale.rangeBand() / 2; 
                var area = d3v2.svg.area()
                    .x(function(d) {return chart.xScale(d.x) + halfBand; }) 
                    .y0(function(d) { return chart.yScale(d.z); })
                    .y1(function(d) { return chart.yScale(d.y); });

                //Area in between
                chart.AreaPlot.objs.g.append("path")
                    .datum(mergedList )
                    .attr("class", "area")
                    .attr("d", area);

                // label lines
                chart.AreaPlot.objs.g.append("text")
                                    .text(dOpts.showLines[0])
                                    .attr("class", "label")
                                    .attr("transform", "translate("+ chart.xScale(mergedList[mergedList.length-1].x) + "," + chart.yScale(mergedList[mergedList.length-1].z)  + ")"); //align texts with boxes;


                chart.AreaPlot.objs.g.append("text")
                                    .text(dOpts.showLines[1])
                                    .attr("class", "label")
                                    .attr("transform", "translate("+ chart.xScale(mergedList[mergedList.length-1].x) + "," + chart.yScale(mergedList[mergedList.length-1].y)  + ")"); //align texts with boxes;

                // legend
                var legend_keys = ["Midspread - 25th and 75th percentiles (IQR)"];

                var lineLegend = chart.AreaPlot.objs.g.selectAll(".lineLegend").data(legend_keys)
                                    .enter().append("g")
                                    .attr("class","lineLegend")
                                    .attr("font-size", "8px")
                                    .attr("transform", function (d,i) {
                                        return "translate(" + (chart.width * 0.5)  + "," + (i*20  +  5)+")";
                                    });
...
/*
            ...
            */    
            ...
            for (cName in chart.groupObjs) {
                ...
                cPlot = chart.groupObjs[cName].AreaPlot;
                cPlot.objs.g = chart.groupObjs[cName].g.append("g").attr("class", "data-plot");
                ...
                if (dOpts.showPlot) {
                    ...
                    for (var pt = 0; pt < chart.groupObjs[cName].values.length; pt++) {
                        cPlot.objs.points.pts.push(cPlot.objs.points.g.append("circle")
                            .attr("class", "point")
                            .attr('r', dOpts.pointSize / 2)// Options is diameter, r takes radius so divide by 2
                            .style("fill", chart.AreaPlot.colorFunct(cName)));
                    }
                }
                ...
                if (dOpts.showBeanLines) {
                    ...
                    for (var pt = 0; pt < chart.groupObjs[cName].values.length; pt++) {
                        cPlot.objs.bean.lines.push(cPlot.objs.bean.g.append("line")
                            .attr("class", "bean line")
                            .style("stroke-width", '1')
                            .style("stroke", chart.AreaPlot.colorFunct(cName)));
                    }
                }
            }
        ...
        d3v2.select(window).on('resize.' + chart.selector + '.dataPlot', chart.AreaPlot.update);
        chart.AreaPlot.update();
        return chart;
    };
